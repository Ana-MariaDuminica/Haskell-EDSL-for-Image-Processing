...
instance Show RegionAST where
    show = showRegion 0
      where
        ...
        showRegion indentLevel (R (Complement x)) = indent indentLevel ++ "~\n" ++ showRegion (indentLevel + 2) x
        showRegion indentLevel (R (Union x y)) = indent indentLevel ++ "+\n" ++ showRegion (indentLevel + 2) x ++ "\n" ++ showRegion (indentLevel + 2) y
        showRegion indentLevel (R (Intersection x y)) = indent indentLevel ++ "*\n" ++ showRegion (indentLevel + 2) x ++ "\n" ++ showRegion (indentLevel + 2) y
        showRegion indentLevel (R (Circle r)) = indent indentLevel ++ "Circle " ++ show r
        showRegion indentLevel (R (Rectangle x y)) = indent indentLevel ++ "Rectangle " ++ show x ++ " " ++ show y
        showRegion indentLevel (R (FromPoints points)) = indent indentLevel ++ "FromPoints " ++ show points
        showRegion indentLevel (R (Transform transformation region)) = indent indentLevel ++ show transformation ++ "\n" ++ showRegion (indentLevel + 2) region
...
instance Functor RegionShape where
    ...
    fmap f (Intersection x y) = Intersection (f x) (f y)
...
basicTransformationCount = foldTransformationAST combiner
  where
    ...
    combiner (Scaling x) = 1
    combiner (Translation x y) = 1
