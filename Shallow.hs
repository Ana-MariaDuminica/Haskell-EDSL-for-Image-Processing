...
plot width height region = 
    let
        x_cord = [-width, (-width) + 1..width]
        y_cord = [height, height - 1..(-height)]
        grid = [[if region(fromIntegral(x), fromIntegral(y)) then '*' else '.' | x <- x_cord] | y <- y_cord]
    in intercalate "\n" grid
...
combineTransformations = foldr (flip (.)) (\x -> x)
...
{-
    *** TODO ***
    ...
    Răspuns: Evaluarea lenesa ne permite sa economisim timp si resurse prin 
    calcularea portiunilor strict necesare pentru a determina apartenenta 
    punctului la regiune. 
    Prin utilizarea evaluarii lenese oprim evaluarea unei subregiuni odata ce se 
    confirma ca punctul se afla acolo, nu este necesar sa continuam evaluarea si 
...
{-
    *** TODO ***
    ...
    Răspuns: Se verifica daca un punct apartine primului cerc din regiune.
    Daca punctul nu apartine primului cerc, evaluarea continua cu urmatoarele 
    cercuri, translatate cu 6 unitati pe orizontala.
    Se continua pana cand se ajunge la o subregiune care contine punctul 
    respectiv sau pana cand se constata ca punctul nu apartine niciunei subregiuni.
-}
