...
toRegion (Union x y) = S.union (toRegion x) (toRegion y)
toRegion (Intersection x y) = S.intersection (toRegion x) (toRegion y)
...
-- transformarile adiacente de acelasi tip sunt fuzionate 
fuseTransformations = foldr combineFunction []
  where
    combineFunction t rest = case rest of
        (t' : rest') -> case (t, t') of
            (Translation x1 y1, Translation x2 y2) ->
                (Translation (x1 + x2) (y1 + y2)) : rest'
            (Scaling s1, Scaling s2) ->
                (Scaling (s1 * s2)) : rest'
            _ ->
                -- daca transformarile nu sunt de acelasi tip sau nu pot fi combinate, vor fi separate in lista
                t : t' : rest'
        -- daca nu exista alte transformari in lista
        [] -> [t]
...
optimizeTransformations region =
  let (transform, optimizedRegion) = simplifyTransformations region
      decomposedTrans = decomposeTransformation transform
      fusedTrans = fuseTransformations decomposedTrans
      finalTransformation = combineTransformations fusedTrans
      -- aplic transformarea finala simplificata asupra regiunii
  in applyTransformation finalTransformation optimizedRegion

simplifyTransformations :: RegionAST -> (TransformationAST, RegionAST)
simplifyTransformations (Transform t r) =
    -- caz pentru Transform
  let (newT, newR) = simplifyTransformations r
  in (combineTransformations [t, newT], newR)
simplifyTransformations (Complement r) =
    -- caz pentru Complement
  let (t, newR) = simplifyTransformations r
  in (t, Complement newR)
simplifyTransformations (Union r1 r2) =
    -- caz pentru Union cu cele doua subregiuni
  let (t1, newR1) = simplifyTransformations r1
      (t2, newR2) = simplifyTransformations r2
  in (t1, Union newR1 newR2)
simplifyTransformations (Intersection r1 r2) =
    -- Intersection cu cele doua subregiuni
  let (t1, newR1) = simplifyTransformations r1
      (t2, newR2) = simplifyTransformations r2
  in (t2, Intersection newR1 newR2)
simplifyTransformations region = (Combine [], region)  -- cazul de baza
